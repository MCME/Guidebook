...
public class GuidebookPlugin extends JavaPlugin {
    ...
    private void initializePlayerMoveRunnable() {
        getServer().getScheduler().scheduleSyncRepeatingTask(
            this,
            () -> {
                List<InfoArea> enabledAreas = PluginData.getInfoAreas().values().stream().filter(InfoArea::isEnable).toList();

                for (Player player : Bukkit.getOnlinePlayers()) {
                    if (PluginData.isExcluded(player)) continue;
